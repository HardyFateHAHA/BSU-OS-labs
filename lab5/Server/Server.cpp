...
int main() {
    ...
    buf += to_string(i);
    tmp = buf.c_str();
    PipeName = new wchar_t[strlen(tmp) + 1];
    mbstowcs(PipeName, tmp, strlen(tmp) + 1);
    STARTUPINFO siC;
    PROCESS_INFORMATION piComC;
    ZeroMemory(&siC, sizeof(STARTUPINFO));
    siC.cb = sizeof(STARTUPINFO);
    CreateProcess(NULL, PipeName, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &siC, &piComC);
    CloseHandle(piComC.hThread);
    CloseHandle(piComC.hProcess);
    delete[] PipeName;
